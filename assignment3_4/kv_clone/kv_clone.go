...
func (t *Temp) VotingRPC(votingStruct raft.VotingStruct, votingStructReply *raft.VotingStructReply) error {   
  //fmt.Println("VotingRPC method: CID,SID,CT,ST -----", votingStruct.CandidateID, raft_obj.ServerId, votingStruct.Term, raft_obj.CurrentTerm) 
  raft_obj.VotingCh<-votingStruct
  temp1 := <-raft_obj.ReplyVotingCh
  *votingStructReply = temp1
  return nil
}

func (t *Temp) AppendLogRPC(appendLogEntry raft.AppendLogEntry, appendLogEntryReply *raft.AppendLogEntryReply) error {  
  //fmt.Println("AppendLogRPC method: CID,SID,CT,ST -----", appendLogEntry.LeaderID, raft_obj.ServerId, appendLogEntry.Term, raft_obj.CurrentTerm)
  raft_obj.AppendLogCh<-appendLogEntry
  temp1 := <-raft_obj.ReplyAppendLogCh
  *appendLogEntryReply = temp1
  return nil
}
...
func main() {
    ...
    ///home/rdp/Dropbox/cloud_cs733/src/cs733/assignment2/server.json
    ///home/rdp/Dropbox/cloud_cs733/bin/server.json
    file, err := ioutil.ReadFile("/home/rdp/Dropbox/cloud_cs733/bin/server.json")
    if err != nil {
        log.Fatal(err)
    }
...
func spawnServers(cc raft.ClusterConfig,sc raft.ServerConfig) {
    ...
    defer wg.Done()
    // Call loop function to start states
    //go loop(raft_obj)
    //defer wg.Done()
