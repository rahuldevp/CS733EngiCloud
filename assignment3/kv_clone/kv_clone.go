...
func (t *Temp) VotingRPC(votingStruct *raft.VotingStruct, votingStructReply *raft.VotingStructReply) error {   
    fmt.Println("VotingRPC method: CandidateID--ServerID -----", votingStruct.CandidateID, raft_obj.ServerId) 
    temp := raft.EventStruct{
      votingStruct.Term,
      votingStruct.CandidateID,
      "VoteRequest",
    }
    raft_obj.EventCh<-temp
    temp1 := <-raft_obj.ReplyEventCh
    temp2 := raft.VotingStructReply{
      temp1.Term,
      temp1.IsVoted,
    }
    fmt.Println("*************",temp2)
    *votingStructReply = temp2
    return nil
}
